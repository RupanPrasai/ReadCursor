 ...
 *   4) local-only fallback: chrome://extensions scraping
 */
...
function resolveUserDataDir(browser: WebdriverIO.Browser): string | null {
  ...
  return pickArgValue(args, '--user-data-dir=');
}
...
async function findPreferencesPath(userDataDir: string): Promise<string | null> {
  const candidates = [
    ...
    path.join(userDataDir, 'Profile 3', 'Preferences'),
  ];
...
async function waitForPreferencesPath(userDataDir: string, timeoutMs = 15_000): Promise<string | null> {
  const started = Date.now();
  while (Date.now() - started < timeoutMs) {
    const p = await findPreferencesPath(userDataDir);
    if (p) return p;
    await delay(250);
  }
  return null;
...
async function resolveChromeExtensionIdViaPreferences(browser: WebdriverIO.Browser): Promise<string | null> {
  ...
  const prefsPath = await waitForPreferencesPath(userDataDir, 15_000);
...
async function resolveChromeExtensionIdViaExtensionsPage(browser: WebdriverIO.Browser): Promise<string> {
  ...
  if (!extensionId) {
    ...
    const debug = await browser.execute(() => {
      ...
      const first = itemList?.shadowRoot?.querySelector('extensions-item') as any;
      return { hasManager: true, hasShadowRoot: true, hasItemList: !!itemList, hasFirstItem: !!first };
    });
...
export const getChromeExtensionPath = async (browser: WebdriverIO.Browser) => {
  ...
  // 1) CI-safe: read from Preferences
  ...
  // 3) Local-only UI scraping (brittle in CI/headless)
  if (isCI()) {
    const args = getChromeArgs(browser);
    const userDataDir = resolveUserDataDir(browser);
    const extDir = resolveLoadedExtensionDir(browser);
    throw new Error(
      `Failed to resolve extension id in CI. userDataDir=${userDataDir} extDir=${extDir} chromeArgs=${JSON.stringify(args)}`,
    );
  }
